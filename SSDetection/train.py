...
def train(cfg, args):
    ...
    device = torch.device(cfg.MODEL.DEVICE) # device = cuda
    model.to(device)
    if args.distributed:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank], output_device=args.local_rank)

    lr = cfg.SOLVER.LR * args.num_gpus  # scale by num gpus # in our case, main learning rate is 0.004
